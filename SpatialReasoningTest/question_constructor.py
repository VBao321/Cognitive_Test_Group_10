...
def set_progress_indicator(question, max):
    progress_indicator["output"]=output
    progress_indicator["question"]=question
    progress_indicator["progress_bar"]=set_progress_bar(max)
    progress_indicator["timer"]=set_timer(question)
...
def update_progress_indicator():
    with output:
        ...
        display_box = widgets.VBox([progress_indicator["progress_bar"],
                                    widgets.HBox([widgets.Label(value="Time left:", layout=widgets.Layout(width = '84px')), progress_indicator["timer"]])])
        display(display_box)
        
    return
        
def set_timer(question):
    if question.timeout != -1:
        timer = widgets.Label(value=f"{question.timeout:.2f} s", layout=widgets.Layout(border='2px solid black',
                                                                                       width = '212px',display='flex',
...
def update_timer():
    timer = progress_indicator["timer"]
    question = progress_indicator["question"]
    timer.value = f"{question.timeout-question.get_time():.2f} s"
...
def update_progress_bar(idx):
    """
    Updates a progress bar as questions are completed.

    Parameters:
        idx (int): Question progress index.

    Returns:
        None
    """
    progress_bar = progress_indicator["progress_bar"]
...
class Question:
    ...
    def wait_for_submission(self, timeout=-1, interval=0.01, max_rate=2000, allow_interupt=True, progress_update_interval=10):
        """
        Waits for user submission, processing UI events during the wait, updating progression indicator.
...
class SpatialReasoningQuestion(Question):  
    ...
    def assemble_option_widgets(self):
        ...
        for option in sorted_options:
            ...
            option_box = widgets.VBox([checkbox, image])
