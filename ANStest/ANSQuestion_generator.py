...
def generate_images(num_points_l, num_points_r):
    ...
    ax.scatter(x_points_l, y_points_l, color='blue', s=150)
    ax.scatter(x_points_r, y_points_r, color='red', s=150)
...
def ANSQuestion_bank(seed):
    ...
    Returns:
        tuple: A tuple containing three lists:
            ...
            2. num_points_list (list): A list of tuples containing the number of points
               generated within the left and right ellipses for each image.
            3. answer_list (list): A list of strings indicating the correct answer for each image,
               where 'left' corresponds to the left ellipse having more points, and 'right'
               corresponds to the right ellipse having more points.
    """
    ...
    for idx in range(64):
        ...
        fig.savefig(f'./Figures/ANSQ_{idx}.png')

        # Add name of image to list
        image_list.append(f'./Figures/ANSQ_{idx}.png')

        # close image
        plt.close(fig)
    
    for idx in range(64):
        output_dict[image_list[idx]]=[answer_list[idx],num_points_list[idx][0],num_points_list[idx][1],num_points_list[idx][0]/num_points_list[idx][1]]
